            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                <div className="bg-white p-7 rounded-lg shadow-xl max-w-xl w-1/2 sm:max-w-full">
                    ...
                    <div className="flex flex-col items-end">
                        <button onClick={onClose} className="px-4 py-2 bg-theme-color-primary text-white rounded-lg hover:bg-theme-color-primary-dark">
                            Close
                        </button>
                    </div>
                </div>
    ...
    const handleStartChallenge = async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                Swal.fire({
                    title: "Error!",
                    html: `${response.message || response.error || "An error occurs, please try again later!"}`,
                    icon: "error",
                    confirmButtonText: "OK"
                });
    ...
    const handleStopChallenge = async () => {
        ...
        try {
            ...
            if (response.isSuccess) {
                ...
                Swal.fire({
                    title: "Oops!",
                    text: errorMessage.includes("Challenge not started or already stopped") ? "This challenge is not currently active or has already been stopped." : errorMessage,
                    icon: "error",
                            ...
                            {challenge ? (
                                <>
                                    <h1 className="text-theme-color-neutral-content text-lg mb-2">
                                        <b>Max attempts</b>: {challenge.max_attempts > 0 ? challenge.max_attempts + " times" : "UNLIMITED"} <br />
                                                ...
                                                <div>
                                                    <div className="flex flex-wrap gap-4">
                                                        {challenge.files.map((file, index) => (
                                                            <button key={index} onClick={() => handleDowloadFiles(file)} className="flex items-center bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <div className="lg:w-[30%] bg-theme-color-base p-8">
                        <div className="mb-8">
                            <div className="flex items-center justify-center space-x-2 text-2xl font-mono bg-white p-4 rounded-lg shadow-md">
                                ...
                                {!isChallengeStarted && <span className="font-bold">{formatTime(timeLimit)}</span>}
                                {isChallengeStarted && <span className="font-bold">{formatTime(timeRemaining)}</span>}
                            </div>
                        </div>
                        {showTimeUpAlert && (
                        ...
                        <div className="space-y-2 mb-4">
                            {hints.length > 0 && <h3 className="font-medium text-theme-color-neutral-content mb-3">Available Hints:</h3>}
                            <div className="grid grid-cols-3 gap-2">
                                {hints.map((hint) => (
                                    <div key={hint.id}>
                                        <button type="button" className="w-full h-16 bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300  items-center justify-center font-medium text-theme-color-primary hover:bg-gray-5" onClick={() => handleUnlockHintClick(hint.id, hint.cost)}>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            {challenge?.type !== "multiple_choice" && (challenge?.attemps < challenge?.max_attempts || challenge?.max_attempts == 0) && !isSubmitted && (
                                <div>
                            ...
                            {(challenge?.attemps <= challenge?.max_attempts || challenge?.max_attempts == 0) && (
                                <button
                                    ...
                                    className={`w-full py-3 px-6 rounded-lg font-medium transition-all duration-200 flex items-center justify-center space-x-2 ${
                                        isSubmitted || (challenge?.require_deploy && !isChallengeStarted) ? "bg-theme-color-neutral cursor-not-allowed" : "bg-theme-color-primary hover:bg-theme-color-primary-dark text-white"
                                    }`}
                            ...
                            {challenge?.require_deploy && !isChallengeStarted && !isSubmitted && (
                                <button
                                    ...
                                    className={`w-full py-3 px-6 rounded-lg font-medium transition-all duration-200 flex items-center justify-center space-x-2 ${isStarting ? "bg-gray-500 text-white cursor-not-allowed" : "bg-green-600 hover:bg-green-700 text-white"}`}
                                >
                            ...
                            {isChallengeStarted && challenge?.require_deploy && !isSubmitted && (
                                <button
                                    ...
                                    className={`w-full py-3 px-6 rounded-lg font-medium transition-all duration-200 flex items-center justify-center space-x-2 ${IsStopping ? "bg-red-400 cursor-not-allowed" : "bg-red-600 hover:bg-red-700 text-white"}`}
                                >
                                    {IsStopping ? (
