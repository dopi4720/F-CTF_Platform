...
from CTFd.models import Hints, HintUnlocks, db, Tokens, Users, Challenges
...
class Hint(Resource):
    ...
    def get(self, hint_id):
        ...
        if auth_header and auth_header.startswith('Bearer '):
            ...
            user =  Users.query.filter_by(id=token_auth.user_id).first() # Hàm validate_token cần được định nghĩa để xác thực token
        else:
...
class HintListByChallenge(Resource):
    ...
    def get(self, challenge_id):
        ...
        if not auth_header or not auth_header.startswith('Bearer '):
            abort(401, description="Unauthorized: Token missing or invalid.")
        ...
        try:
            ...
            hints_list = Hints.query.filter_by(challenge_id=challenge_id).all()
            
            serialized_hints = {
                "size": len(hints_list),
                "hints": [{"id":hint.id,"cost":hint.cost} for hint in hints_list]
            }

            return {"hints": serialized_hints}, 200
